...
function findWaldo(crowd) {
    const s = crowd.join(''), choices = new Set([...s])
    const waldo = [...choices].filter(c => s.replace(new RegExp(c, 'g'), '').length == s.length - 1)[0]
    const y = crowd.findIndex(r => r.includes(waldo))
